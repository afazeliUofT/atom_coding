...
Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (key == "--warmup-trials") a.warmup_trials = std::stoi(need());
...
void write_header(std::ofstream& out) {
    out << "family,code_name,rate,n,k,p,trial,error_weight,deleted_position,agreement,fiber_certified,prefix_certified,trellis_available,trellis_certified,"
        ...
        << "prefix_nodes,prefix_membership,prefix_exact_scores,prefix_peak_frontier,trellis_estimated_updates,trellis_nodes,trellis_updates,trellis_terminals,trellis_exact_scores\n";
}
        ...
        for (double rate: a.rates) {
            for (int n: a.ns) {
                ...
                for (double p: a.ps) {
                    ...
                    const long double trellis_estimated_updates_ld = 2.0L * code.n * code.n * (uint64_t(1) << code.r);
                    const uint64_t trellis_estimated_updates =
                        trellis_estimated_updates_ld > static_cast<long double>(std::numeric_limits<uint64_t>::max())
                            ? std::numeric_limits<uint64_t>::max()
                            : uint64_t(trellis_estimated_updates_ld);
                    std::cout << "compiled family=" << code.family << " R=" << double(k)/n << " n=" << n << " p=" << p
                              << " trials=" << trials << " trellis_available=" << (trellis_available?1:0)
                              << " trellis_estimated_updates=" << trellis_estimated_updates << std::endl;

                    // Per-configuration warmup is excluded from the reported timing.
                    for (int warm = 0; warm < a.warmup_trials; ++warm) {
                        Sample ws = sample_channel(code, p, rng);
                        DecodeResult wf = fiber_decode(ws.received, code, p, a.max_histories);
                        DecodeResult wp = prefix_decode(ws.received, code, p, a.max_prefix_nodes);
                        if (!wf.certified || !wp.certified || !same_ties(wf.ties, wp.ties))
                            throw std::runtime_error("warmup decoder disagreement");
                        if (trellis_available) {
                            DecodeResult wt = syndrome_trellis_decode(ws.received, code, p, a.max_trellis_terminals, a.max_trellis_dp_updates);
                            if (!wt.certified || !same_ties(wf.ties, wt.ties))
                                throw std::runtime_error("warmup trellis disagreement");
                        }
                    }
                    ...
                    for (int trial = 0; trial < trials; ++trial) {
                        ...
                        if (trellis_available) decoder_order.push_back(2);
                        ...
                        out << code.family << ',' << '"' << code.name << '"' << ',' << double(k)/n << ',' << n << ',' << k << ',' << p << ',' << trial << ','
                            ...
                            << trellis_estimated_updates << ',' << tr.work.trellis_nodes << ',' << tr.work.trellis_updates << ',' << tr.work.trellis_terminals << ',' << tr.work.exact_scores << '\n';
                        rows++;
                    }
                }
            }
