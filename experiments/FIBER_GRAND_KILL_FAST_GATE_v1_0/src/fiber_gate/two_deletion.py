...
def run_two_deletion_scaling(
    ...
    for family in code_families:
        for n in blocklengths:
            ...
            code = make_code(family, n, k, rng, label="_D2")
            trials = int(trial_schedule.get(str(n), 16))
            for p in substitution_probabilities:
                channel = DeletionChannel(n=n, deletions=2, substitution_probability=float(p))
    ...
    frame.to_csv(output_dir / "06_two_deletion_trials.csv.gz", index=False, compression="gzip")
    path_frame.to_csv(output_dir / "06_two_deletion_pathwise.csv.gz", index=False, compression="gzip")
    ...
    pd.DataFrame(summary_rows).to_csv(output_dir / "06_two_deletion_summary.csv", index=False)
    path_summary = {
        f"{family}|p={p}": {
            "trials": len(group),
    ...
    write_json(output_dir / "06_two_deletion_scaling.json", payload)
    return payload
