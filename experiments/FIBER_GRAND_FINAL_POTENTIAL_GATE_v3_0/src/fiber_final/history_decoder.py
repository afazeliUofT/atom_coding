...
def _strict_certificate(best: float, residual: float, tolerance: float) -> bool:
    # Conservative outward rounding.  The exact micro-world audit remains the
    # authoritative correctness check.
    lower = math.nextafter(float(best), -math.inf)
    upper = math.nextafter(float(residual), math.inf)
    margin = tolerance * max(1.0, abs(lower), abs(upper))
...
def history_decode(
    ...
    while heap and work.histories < max_histories:
        ...
        work.histories += 1
        work.candidate_build_ops += max(1, channel.n // 8 + channel.deletions)
        ...
        if best_words and _strict_certificate(best, residual, tolerance):
            certified = True
            break
