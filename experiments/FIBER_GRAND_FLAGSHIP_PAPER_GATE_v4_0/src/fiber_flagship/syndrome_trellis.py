...
class AlignmentCodewordStream:
    ...
    def __init__(self, received: int, deleted_position: int, channel: FixedDeletionBSC, code: LinearOracle, work: Work) -> None:
        ...
        self.target = target
        states = 1 << self.r
        state_index = np.arange(states, dtype=np.int64)
        inf = self.n + 1
        ...
        for pos in range(self.n - 1, -1, -1):
            ...
            dp[pos] = np.minimum(c0 + dp[pos + 1], c1 + dp[pos + 1, state_index ^ column])
            work.trellis_dp_updates += 2 * states
        self.dp = dp
        self.heap: list[tuple[int, int, int, int, int]] = []
        self.serial = 0
        root_h = int(dp[0, 0])
        heapq.heappush(self.heap, (root_h, 0, self.serial, 0, 0))
        self.serial += 1
        work.heap_pushes += 1
    ...
    def next(self, work: Work) -> tuple[int, int] | None:
        nmask = (1 << self.n) - 1
        while self.heap:
            _, g, _, pos, packed = heapq.heappop(self.heap)
            work.heap_pops += 1
...
def syndrome_trellis_aggregate_decode(
    ...
    while heap and work.trellis_terminals <= max_terminals:
        _, j, _, word, _ = heapq.heappop(heap)
        work.heap_pops += 1
        old_eta = eta[j]
        ...
        if word in seen:
            ...
            if score > best + tolerance:
                ...
                best_words.add(word)
        work.bound_checks += 1
        if best_words and math.nextafter(best, -math.inf) > math.nextafter(residual, math.inf) + tolerance:
            certified = True
            break
