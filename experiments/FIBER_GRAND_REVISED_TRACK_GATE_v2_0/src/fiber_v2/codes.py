...
class VTOracle:
    ...
    @property
    def rate(self) -> float:
        # Asymptotic/checksum proxy; exact size is deliberately not enumerated.
    ...
    def sample_codeword(self, rng: np.random.Generator) -> int:
        # Expected n+1 trials; no codebook table is constructed.
...
def crc_like(n: int, k: int, polynomial: int | None = None, label: str = "") -> LinearOracle:
    r = n - k
    if polynomial is None:
        # Deterministic primitive-ish odd polynomial of requested degree.
        polynomial = (1 << r) | (1 << max(1, r // 2)) | 1
    ...
    if not np.array_equal(g[:, :k], np.eye(k, dtype=np.uint8)):
        g = systematicize(g)
    return LinearOracle(f"CRC_LIKE_n{n}_k{k}_poly{polynomial:x}{label}", "crc_like", g)
...
def make_linear(family: str, n: int, k: int, rng: np.random.Generator, label: str = "") -> LinearOracle:
    ...
    if normalized == "CRC":
        return crc_like(n, k, label=label)
